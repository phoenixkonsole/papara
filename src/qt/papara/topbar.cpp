...
TopBar::TopBar(paraGUI* _mainWindow, QWidget *parent) :
    ...
    ui(new Ui::TopBar)
{
    ...
    setCssProperty({ui->labelAmountTopPiv, ui->labelAmountTopzpara}, "amount-small-topbar");
    setCssProperty({ui->labelAmountPiv, ui->labelAmountzpara}, "amount-topbar");
    setCssProperty({ui->labelPendingPiv, ui->labelAmountPivLocked, ui->labelPendingzpara, ui->labelImmaturePiv, ui->labelImmaturezpara}, "amount-small-topbar");
...
void TopBar::updateBalances(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance,
    ...
    QString totalzpara = GUIUtil::formatBalance(matureZerocoinBalance, nDisplayUnit, true);
    // Top
    ui->labelAmountTopPiv->setText(totalPiv);
    ui->labelAmountTopzpara->setText(totalzpara);

    // Expanded
    ui->labelAmountPiv->setText(GUIUtil::formatBalance(pivAvailableBalance - nLockedBalance, nDisplayUnit));
    ui->labelAmountzpara->setText(totalzpara);
    ...
    ui->labelPendingzpara->setText(GUIUtil::formatBalance(unconfirmedZerocoinBalance, nDisplayUnit, true));

    ui->labelImmaturePiv->setText(GUIUtil::formatBalance(immatureBalance, nDisplayUnit));
    ui->labelImmaturezpara->setText(GUIUtil::formatBalance(immatureZerocoinBalance, nDisplayUnit, true));
}
