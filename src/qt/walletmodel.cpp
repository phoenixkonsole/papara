...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 para.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal papara:URI (papara:XyZ...?message=example)
            {
...
bool WalletModel::createZparaSpend(
        CWalletTx &wtxNew,
...
bool WalletModel::convertBackZpara(
        CAmount value,
        std::vector<CZerocoinMint> &vMintsSelected,
        bool fMintChange,
        bool fMinimizeChange,
    ...
    if(!isMine(addressTo)){
        receipt.SetStatus(_("To convert zpara back to para the return address must be from your wallet"), 1);
        return false;
    }
...
static void NotifyzparaReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzparaReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzparaReset.connect(boost::bind(NotifyzparaReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzparaReset.disconnect(boost::bind(NotifyzparaReset, this));
