                                                ...
                                                fDenomsMinimized(true)
{
    ...
    // "Spending 999999 zpara ought to be enough for anybody." - Bill Gates, 2017
    ui->zparapayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzparaSyncStatus->setText("(" + tr("out of sync") + ")");
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zparapayAmount->setFocus();
    }
}
/* disable MINT
 *
void PrivacyDialog::on_pushButtonMintzpara_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zpara is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Mint_zpara, true));
        if (!ctx.isValid()) {
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zpara...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zpara in ") +
...
void PrivacyDialog::on_pushButtonSpendzpara_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zpara is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        WalletModel::UnlockContext ctx(walletModel->requestUnlock(AskPassphraseDialog::Context::Send_zpara, true));
        if (!ctx.isValid()) {
            // Unlock wallet was cancelled
            return;
        }
        // Wallet is unlocked now, sedn zpara
        sendzpara();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zpara
    sendzpara();
}

void PrivacyDialog::on_pushButtonZparaControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
...
void PrivacyDialog::setZparaControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzparaSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzpara()
{
    ...
    double dAmount = ui->zparapayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zpara is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " para </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zpara</b>";
    ...
    if (walletModel && walletModel->getAddressTableModel()) {
        // If zpara was spent successfully update the addressbook with the label
    ...
    // Clear zpara selector in case it was used
    ZparaControlDialog::setSelectedMints.clear();
    ui->labelzparaSelected_int->setText(QString("0"));
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zpara Spend #: ") + QString::number(nCount) + ", ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zpara </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zpara "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zpara "));
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zpara "));
    ...
    ui->labelZsupplyAmount->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zpara </b> "));
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zpara </b> "));
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        ...
        QString strSupply = QString::number(nSupply) + " x " + QString::number(denom) + " = <b>" +
                            QString::number(nSupply*denom) + " zpara </b> ";
        switch (denom) {
            case libzerocoin::CoinDenomination::ZQ_ONE:
...
{
    ui->labelzparaSyncStatus->setVisible(fShow);
}
...
void PrivacyDialog::updateSPORK16Status()
{
    ...
    //bool fButtonsEnabled =  ui->pushButtonMintzpara->isEnabled();
    ...
    if (fMaintenanceMode && fButtonsEnabled) {
        // Mint zpara
        //ui->pushButtonMintzpara->setEnabled(false);
        //ui->pushButtonMintzpara->setToolTip(tr("zpara is currently disabled due to maintenance."));

        // Spend zpara
        ui->pushButtonSpendzpara->setEnabled(false);
        ui->pushButtonSpendzpara->setToolTip(tr("zpara is currently disabled due to maintenance."));
    } else if (!fMaintenanceMode && !fButtonsEnabled) {
        // Mint zpara
        //ui->pushButtonMintzpara->setEnabled(true);
        //ui->pushButtonMintzpara->setToolTip(tr("PrivacyDialog", "Enter an amount of para to convert to zpara", 0));

        // Spend zpara
        ui->pushButtonSpendzpara->setEnabled(true);
        ui->pushButtonSpendzpara->setToolTip(tr("Spend Zerocoin. Without 'Pay To:' address creates payments to yourself."));
    }
}
