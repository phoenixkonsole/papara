...
unsigned int GetMasternodeTierFromOutput(CAmount nValue, int blockHeight)
{
    ...
    if (blockHeight < TIER_BLOCK_HEIGHT) {
        ...
        for (unsigned int tier = MasternodeTiers::TIER_100; tier <= MasternodeTiers::TIER_10K; ++tier) {
            if (nValue == MASTERNODE_TIER_COINS[tier] * COIN) {
                tierRet = tier;
                break;
            }
...
{
    if (nTier >= MasternodeTiers::TIER_NONE || nTier < MasternodeTiers::TIER_100) {
        return 0;
    }
    else {
...
{
    const unsigned int distribution[MasternodeTiers::TIER_NONE] = {1, 3, 10, 30, 100, 300, 1000, 3000, 10000, 30000, 100000};
